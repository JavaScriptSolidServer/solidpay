...
export class Ledger {
  ...
  setTrustline(actor, peer, currency, limit) {
  ...
  pay(actor, to, currency, amount) {
    ...
    const entry = this._transition(actor, 'send-payment',
      { from: actor, to, currency: cur, amount: fromMicro(amt), path: p });
  ...
  settle(actor, peer, currency, amount) {
    peer = normalizeAgent(peer);
    if (!peer) bad('peer required');
    const cur = this._currency(currency);
    const amt = this._amount(amount);
    ...
    const entry = this._transition(actor, 'settle', { peer, currency: cur, amount: fromMicro(amt) });
