...
export function uiPage(origin = '') {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>SolidPay — money is trust</title>
<meta name="description" content="A SolidPay testnet node: extend credit to people you trust, pay anyone your network can reach. Every user is a URI, every transition is a signed nostr event, the ledger proves itself.">
<meta property="og:type" content="website">
<meta property="og:site_name" content="SolidPay">
<meta property="og:title" content="SolidPay testnet — money is trust">
<meta property="og:description" content="Extend credit, pay through chains of trust. Signed transitions, a self-proving ledger, Bitcoin-anchored history. Play money, real protocol.">
...
<meta name="twitter:title" content="SolidPay testnet — money is trust">
...
function signedTx(type,intent){
  ...
  return window.nostr.signEvent({kind:TXKIND[type],created_at:Math.floor(Date.now()/1000),tags:[['nonce',nonce]],content:jcs(intent)})
...
function lineAction(ev){
  ...
  /* Inline editor replaces the row's action cell — no browser dialogs. */
  var cell=b.parentElement;
  var isEdit=act==='edit';
  var init=isEdit?b.dataset.lim:b.dataset.max;
  var label=isEdit?'New limit':'Amount repaid';
  cell.dataset.prev=cell.innerHTML;
  cell.innerHTML='<span style="display:inline-flex;gap:6px;align-items:center">'
    +'<input type="number" min="0" step="any" value="'+esc(init)+'" aria-label="'+label+'" '
    +'style="width:110px;padding:5px 8px;border:1px solid var(--acc);border-radius:7px;background:var(--surface2);color:var(--ink);font-size:12.5px">'
    +'<button class="btn sm primary" data-x="ok">'+(isEdit?'Set':'Record')+'</button>'
    +'<button class="btn sm quiet" data-x="no">Cancel</button></span>';
  var input=cell.querySelector('input');input.focus();input.select();
  function done(){cell.innerHTML=cell.dataset.prev;bindLineActions(cell)}
  cell.querySelector('[data-x=no]').onclick=done;
  function submit(){
    var n=Number(input.value);
    if(!(n>=0)){input.style.borderColor='var(--neg)';return}
    var p=isEdit
      ?txOrApi('set-trustline',{peer:peer,currency:cur,limit:n},'/trustlines')
      :txOrApi('settle',{peer:peer,currency:cur,amount:n},'/settle');
    p.then(function(r){toast(r.ok?(isEdit?'Limit updated':'Repayment recorded'):(r.body.error||'failed'));refresh()});
  }
  cell.querySelector('[data-x=ok]').onclick=submit;
  input.addEventListener('keydown',function(e){if(e.key==='Enter')submit();if(e.key==='Escape')done()});
}
function bindLineActions(scope){
  (scope||document).querySelectorAll('button[data-act]').forEach(function(b){b.onclick=lineAction});
}
